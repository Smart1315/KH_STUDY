...
<body>
	<div class="wrapper">
		...
		<div class="main">
			...
			<main id="content">
				<h2 style="margin-left:20px;">매칭</h2>
					<div>
						<table class="board_list">
							...
							<tbody>	
							<c:if test="${ requestScope.mcList.size() <= 0 }">
								<tr>
									<td colspan="5">
										조회 결과가 없습니다.
									</td>
							...
							<c:if test="${ requestScope.mcList.size() > 0 }">
								<c:forEach items="${ requestScope.mcList }" var="mc">
								<tr>
									<td>${mc.matchNum }</td>
									<td style="text-align:center;">
										<c:if test="${ mc.matchCate == 1 }">
											짝
										</c:if>
										<c:if test="${ mc.matchCate == 2 }">
											산책 메이트
										</c:if>
										<c:if test="${ mc.matchCate == 3 }">
											펫 시터
										</c:if>
									</td>
									<td>${ mc.matchTitle }</td>
									<td>${ mc.memberId }</td>
									<td>													
										<c:url var="matchDelete" value="matchDelete.mn">
											<c:param name="matchNum" value="${ mc.matchNum }"/>
											<c:param name="cate" value="${ mc.matchCate}"/>
										</c:url>
										<a class="fas fa-times" href="${ matchDelete }"></a>	
									</td>
								</tr>
								</c:forEach>
							</c:if>
