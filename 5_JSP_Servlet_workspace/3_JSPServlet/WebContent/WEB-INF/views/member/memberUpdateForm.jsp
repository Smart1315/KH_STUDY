...
<%
	...
	String interests = myInfo.getInterest();
	if(interests != null) {
		String[] splitStr = interests.split(", ");
		
		for(int i = 0; i < splitStr.length; i++) {
			switch(splitStr[i]) {
			case "운동" :
				checkedInterest[0] = "checked";
				break;
			case "등산" :
				checkedInterest[1] = "checked";
				break;
			case "낚시" :
				checkedInterest[2] = "checked";
				break;
			case "요리" :
				checkedInterest[3] = "checked";
				break;
			case "게임" :
				checkedInterest[4] = "checked";
				break;
			case "기타" :
				checkedInterest[5] = "checked";
				break;
			}
		}
	}
		...
		<form action="<%= request.getContextPath() %>/update.me" method="post" id="updateForm" name="updateForm">
			<table>
				...
				<tr>
					<td>연락처</td>
					<td>
						<input type="tel" maxlength="11" name="myPhone" placeholder="(-없이)01012345678" value="<%= myInfo.getPhone() == null ? "" : myInfo.getPhone() %>">
					</td>
				</tr>
				<tr>
					<td>이메일</td>
				...
				<tr>
					<td>주소</td>
					<td><input type="text" name="myAddress" value="<%= myInfo.getAddress() == null ? "" : myInfo.getAddress() %>"></td>
				</tr>
				<tr>
					<td>관심분야</td>
					<td>
