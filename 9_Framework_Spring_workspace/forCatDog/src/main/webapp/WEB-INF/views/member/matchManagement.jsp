...
<link rel="stylesheet" type="text/css" href="${ contextPath }/resources/css/member/matchManagement.css">
</head>
<body>
	<div class="wrap">
					...
					<c:if test="${requestScope.list.size() > 0 }">
						...
						<c:forEach items="${ requestScope.list }" var="mc">
							<tr style="background: #F2EBEB;">
								...
								<c:forEach items="${ mc.mList }" var="m">
									<tr>
										<td></td>
										<td class="req">
											<span class="reqRow" onclick="reqInfo(${ mc.matchNum }, '${ m.memberId }', '${ m.nickName }');">${ m.matchContent }</span>
											<c:if test="${ option == 'Y' }">
												<div class="chatBtnBlock">
													<button type="button" class="chat" onclick="reqInfo(${ mc.matchNum }, '${ m.memberId }', '${ m.nickName }', '${ option }');">
	...
	<script>
		$(function() {
			$(".chatBtnBlock").siblings("span").addClass("reqLink");
		});
	
		function matchList(option) {
			location.href = '${ contextPath }/matchList.me?option=' + option;
		}
