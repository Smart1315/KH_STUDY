			...
			<main id="content">
					<div>
						<table class="board_list">
							...
							<c:if test="${ requestScope.ccList.size() > 0 }">
								<c:forEach items="${ requestScope.ccList }" var="cc">
								<tr>
									<td>${cc.cd_num }</td>
									<td style="text-align:center;">${ cc.cd_cate }</td>
									<td>${ cc.cd_name }</td>
									<td>Admin</td>
									<td>						
										<c:url var="catdogUpdate" value="catdogUpdateForm.cc">
											<c:param name="cdNum" value="${ cc.cd_num }"/>
										</c:url>
										<a class="fas fa-pen" href="${ catdogUpdate }"></a>
										
										<c:url var="catdogDelete" value="catdogDelete.cc">
											<c:param name="cdNum" value="${ cc.cd_num }"/>
										</c:url>
										<a class="fas fa-times" href="${ catdogDelete }"></a>	
